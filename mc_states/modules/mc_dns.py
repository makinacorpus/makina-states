...
def __virtual__():
    if not HAS_IPWHOIS:
        return False, ['no ipwhois']
    if not HAS_PYWHOIS:
        return False, ['no pywhois']
    if not HAS_SUDS:
        return False, ['no suds']
    if not HAS_OVH:
        return False, ['no ovh']
    return 'mc_dns'
...
def register_dns_masters(only_domains=None, only_providers=None):
    ...
    try:
        skipped = __salt__['mc_pillar.query']('skipped_updated_domains')
    except Exception:
        skipped = []
