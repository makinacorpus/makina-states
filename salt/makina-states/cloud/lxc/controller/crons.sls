...
remove-syncron-lxc-ms:
  cron.absent:
    ...
    - name: /usr/bin/salt-run -linfo mc_lxc.sync_images > /dev/null
...
syncron-lxc-ms:
  file.managed:
    ...
    - group: root
    - contents: |
                {{settings.lxc.cron_minute}} {{settings.lxc.cron_hour}} * * * /usr/bin/salt-run -linfo mc_lxc.sync_images > /dev/null
{% endif %}
