...
setup_virtualenv() {
    ...
    if is_salt_bins_in_place ${skind} && check_py_modules ${skind};then
        ...
        if [ "x${install_git}" != "x" ];then
            ...
            die_in_error "requirements/git_saltèrequirements.txt doesnt install"
        else
            cwd="${PWD}"
            for i in ansible docker-py salttesting salt m2crypto;do
...
install_nodetype() {
    if [ "x${DO_NODETYPE_INSTALL}" != "x" ];then
        ...
        bs_log "Nodetype bootstral for $(get_salt_nodetype) skipped"
    fi
}
